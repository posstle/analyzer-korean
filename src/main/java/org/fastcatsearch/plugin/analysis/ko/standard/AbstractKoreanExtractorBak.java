...
public abstract class AbstractKoreanExtractorBak {
	protected static Logger logger = LoggerFactory.getLogger(AbstractKoreanExtractorBak.class);

	protected CommonDictionary<TagProb, PreResult<TagCharVector>> koreanDic;
	...
	public AbstractKoreanExtractorBak(CommonDictionary<TagProb, PreResult<TagCharVector>> koreanDic, int tabularSize) {
	...
	public CommonDictionary<TagProb, PreResult<TagCharVector>> dictionary(){
		return koreanDic;
	}
	
	private void initConnectionTable() {
		connectionTable = new int[PosTag.values().length][];
		logger.debug("initConnectionTable size={}", connectionTable.length);
...
//		return connectionTable[last.posTag().ordinal()][next.posTag().ordinal()] == 1;
	...
	public void setKoreanDic(CommonDictionary<TagProb, PreResult<TagCharVector>> koreanDic) {
		this.koreanDic = koreanDic;
	}
...
//					tabular[row][column] = chainedEntry;
					for (int i = 0; i < tagList.size(); i++) {
						...
						//성능튜닝!!!
						//어미가 발견되면 포함한 내부어미는 삭제한다.
						//어미는 선어말어미등이 결합되어 긴어미를 형성할 경우 더 세분화해서 분석할 필요가없다.
						if(tagProb.posTag() == PosTag.E){
							//해당단어내에 매칭된 어미 단어는 지워준다.
							if(column > 1){
								removeTagCascade(row, column, PosTag.E);
							}
						}
						if(tagProb.posTag() == PosTag.J){
							//해당단어내에 매칭된 조사 단어는 지워준다.
							if(column > 1){
								removeTagCascade(row, column, PosTag.J);
							}
						}
					}
					status[row]++;
